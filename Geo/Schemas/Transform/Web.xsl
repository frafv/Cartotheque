			...
			<script language="javascript">
				<![CDATA[
					...
					function Transform(xmlDoc,xslDoc) {
						if(!detectIE() && window.XSLTProcessor) {
...
<xsl:template match="*" mode="ViewLanguage">
	...
	<xsl:param name="External" select="//*[e:Prefix/@External = 'lang']/*[@e:Name = $Lang or contains($Lang,'-') and @e:Name = substring-before($Lang,'-')]"/>
	<xsl:variable name="UILang">
		<xsl:apply-templates select="." mode="UILang"/>
	</xsl:variable>

	<xsl:for-each select="$External[1]">
		<xsl:choose>
